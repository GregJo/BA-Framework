...
- countTexture: Overdraw count on fragments -> count of overlapping textures

Update 1 (06.09.2015, 03:30): Weighted average is now working properly now.
...
int main()
{
	...
	// Load accumulation shader for accumulating all the transparent matrials, as well as their alphas.
	GLSLProgram* accumTransparencyShader = new GLSLProgram();
	accumTransparencyShader->initShaderProgram("AccumTransparencyVert.glsl","","","","AccumTransparencyFrag.glsl");
	...
	GLSLProgram* weightedAverageShader = new GLSLProgram();
	weightedAverageShader->initShaderProgram("WeightedAverageVert.glsl","","","","WeightedAverageFrag.glsl");
	...
	screenFillingQuad->setGLSLProgram(*weightedAverageShader);
	...
	while(!window->shouldClose())
	{
		...
		model->setGLSLProgram(accumTransparencyShader);
		quad1->setGLSLProgram(*accumTransparencyShader);
		quad2->setGLSLProgram(*accumTransparencyShader);
		quad3->setGLSLProgram(*accumTransparencyShader);
