...
int main()
{
	...
	// Framebuffer for transparent meshes.
	Framebuffer* frameBufferTransparent = new Framebuffer(window->getWindowWidth(),window->getWindowHeight());
	frameBufferTransparent->setColorAttachment(0);
	//frameBufferFinal->setDepthTextureOnly();
	frameBufferTransparent->setDepthStencilTexture();
	frameBufferTransparent->unbind();
	...
	while(!window->shouldClose())
	{
		...
		frameBufferOpaque->bindForRenderPass(activeColorAttachments_allFBOs);
		...
		/*
		// Transparent pass default OpenGL version.
		frameBufferTransparent->bind();
		frameBufferTransparent->bindForRenderPass(activeColorAttachments_allFBOs);
		...
		*/
		...
		// Transparent pass depth peeling version.
		// Depth peeling is used. (transparent only)
		...
		for (int i = numOfDepthPeelPasses; i >= 0; i--)
		{
			screenFillingQuad->getCurrentShaderProgram()->use();
