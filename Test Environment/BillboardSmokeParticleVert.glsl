...
uniform vec3 relativeParticleCenterPosition = vec3(0,0,0);//vec3(0,0,0);
...
void main()
{
	...
	vec3 scaledWorldspaceVertexPosition = worldspaceParticleCenter + relativeParticleCenterPosition
									 + worldspaceCameraRightVector * worldspaceVertexPosition.x
									 + vec3(0,worldspaceVertexPosition.y,0);

	gl_Position = VPMatrix*vec4(scaledWorldspaceVertexPosition,1.0);
}