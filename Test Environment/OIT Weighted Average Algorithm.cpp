									...
									GLuint sampler)
{
	...
	m_accumTransparencyShader->setUniform("scaleCoeff", m_globalScale);
...
void OITWeightedAverage::VAlgorithm(FreeCamera* camera, std::vector<Model*> models, std::vector<Quad*> transparentQuads, std::vector<Quad*> transparentBillboards,
									Framebuffer* frameBufferOpaque, GLSLProgram* defaultShader, 
									GLuint sampler)
{
	frameBufferOpaque->bind();
	...
	m_accumTransparencyBillboardsShader->use();
	m_accumTransparencyBillboardsShader->setUniform("scaleCoeff", m_globalScale);
	m_accumTransparencyBillboardsShader->setUniform("VPMatrix", camera->getVPMatrix());
	m_accumTransparencyBillboardsShader->setUniform("worldspaceCameraRightVector", camera->getRightVector());
	m_accumTransparencyBillboardsShader->setUniform("worldspaceCameraUpVector", camera->getUpVector());
	for (int i = 0; i < transparentBillboards.size(); i++)
	{
		m_accumTransparencyBillboardsShader->setUniform("worldspaceParticleCenter", transparentBillboards[i]->getPosition());
		transparentBillboards[i]->setGLSLProgram(*m_accumTransparencyBillboardsShader);
	}
...
void OITWeightedAverage::VAlgorithm(FreeCamera* camera, std::vector<Model*> models, std::vector<Quad*> transparentQuads, std::vector<SmokeParticleSystem*> smokeParticleSystems,
				Framebuffer* frameBufferOpaque, GLSLProgram* defaultShader, 
				GLuint sampler)
{
	frameBufferOpaque->bind();
	...
	{
		smokeParticleSystems[i]->render(m_accumTransparencySmokeParticleSystemShader,camera,m_width,m_height);
	}
