...
MeshData::MeshData(const GLfloat* position, const GLfloat* texcoord, 
...
// Data is interleaved.
GLfloat* MeshData::getMeshVertDataAsArrayInterleaved()
{
...
bool* MeshData::getMeshState()
{
	bool* state = new bool[5];
	state[0]=m_position_set;
	state[1]=m_texcoord_set;
	state[2]=m_normals_set;
	state[3]=m_color_set;
	state[4]=m_indices_set;
	return state;
}